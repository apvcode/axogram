...
public class PermissionRequest {
    private static final String PREF_CONTACTS_DONT_ASK = "contacts_permission_dont_ask";
    ...
    public static void ensureEitherPermission(int iconResId, int stringResId, String[] checkPermissions, String[] requestPermissions, Utilities.Callback<Boolean> whenDone) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (needsPermissionRationale) {
                if (stringResId == R.string.ContactsPermissionAlert && MessagesController.getGlobalNotificationsSettings().getBoolean(PREF_CONTACTS_DONT_ASK, false)) {
                ...
                if (stringResId == R.string.ContactsPermissionAlert) {
                    builder.setButton(AlertDialog.BUTTON_NEGATIVE_2, LocaleController.getString(R.string.ContactsPermissionAlertDontAsk), (dialog, which) -> {
                        MessagesController.getGlobalNotificationsSettings().edit().putBoolean(PREF_CONTACTS_DONT_ASK, true).commit();
                    });
                }
                builder.create().show();
                if (whenDone != null) whenDone.run(false);
